...
export class ListVotesComponent implements OnInit {
  ...
  public displayString(dynamicString: IDynamicValue<string>): string {
    return dynamicString.status === RetrievalStatus.available ?
      dynamicString.value :
  ...
  private initialiseSelectedContract$(): Observable<number> {
    return this.rowClicked$
      ...
      .filter(summary => summary.phase.status === RetrievalStatus.available)
      .filter(summary => summary.topic.status === RetrievalStatus.available)

...
export class ReplacementAnonymousVotingContractService implements IReplacementAnonymousVotingContractService {
  at(addr: address): IAnonymousVotingContractManager {
    return new MockAnonymousVotingContractManager(addr);
  }
}

class MockAnonymousVotingContractManager implements IAnonymousVotingContractManager {
  ...
  get phase$(): Observable<number> {
    return this._collection$
      ...
      .switchMap(phase => Observable.range(0, phase + 1))
